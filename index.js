...
function get20s(array){
  // Create new array
  tweniethCenturyArray = [];
  // Loop through the array
  for (let i = 0; i < array.length; i++) {
    let yearsSplit = array[i].years.split(" -");
    if(Number(yearsSplit[0]) >= 1900 && Number(yearsSplit[1]) < 2000) {
      tweniethCenturyArray.push(array[i].name);
    }
  }
  return tweniethCenturyArray;
}
...
console.log(addArtist(artists, 20, "Nathan", "1994 - 2020", "WebDev", "American", "25 years old, loves computers, currently studying to become a web developer"));
